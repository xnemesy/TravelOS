...
- **Correzione obbligatoria rispetto allo stato attuale**: eliminare la coppia di alias duplicati. Un solo nome canonico per livello: `external` (non più `baseData`+`external`), `personal` (non più `memories`+`personal`). Questa non è una questione TypeScript: è la stessa causa radice dell'ambiguità già presente nel modello, semplicemente non ancora emersa come errore di compilazione.
- **Proprietà consentite**: i tre livelli (`external: ExternalPlace`, `editorial?: EditorialPlace`, `personal?: PlaceMemories`), più metadati di identità/provenienza (`id`, `tripId`, `externalProviderId`, `source`), più i campi di pianificazione **dichiarativi** (non calcolati) che sopravvivono a una sessione di planning: `priority`, `status`, `assignedDay`. **Non** i campi calcolati a runtime (`calculatedStartTime`/`calculatedEndTime`, `distanceMeters`, `warnings`) — quelli appartengono solo a `PlaceRef`.
...
- **Chi può crearlo**: esclusivamente `PlaceMergeEngine.mergeFromProvider(existing: TravelPlace | null, incoming: PlaceMetadata): TravelPlace` — punto di ingresso unico, sia per un nuovo salvataggio sia per un aggiornamento del livello External di un luogo già salvato. Nessun altro modulo costruisce un `TravelPlace` per object literal. Questo metodo **non è nuovo**: è la standardizzazione di `PlaceMergeEngine.mergePlace()` (già esistente, §2.8), con l'unione di input ridotta al solo `PlaceMetadata` e senza i cast `as PlaceMetadata` oggi necessari per disambiguare un input a doppia forma.
- **Chi può modificarlo**:
  - Livello `external`: solo `PlaceMergeEngine.mergeFromProvider`, mai altrove.
  ...
  - Livello `personal`: solo azioni utente esplicite instradate attraverso `PlacesEngine` (diario, rating personale, check-in, preferiti) — mai un motore di pianificazione.
...
```
   ...
   (catalogo curato, statico,             │                (sostituisce Place+PlaceMetadata duplicati, §3.5)
    livello Editorial pre-popolato)       │
                          ...
                          tocca SOLO il livello `external` — Editorial/Personal preservati
                                          │
                                          ▼
        ┌─────────────────────────────────────────────────────────────┐
        │                          TravelPlace                          │
...
|---|---|---|---|---|
| 1 | `src/domain/trip/models/place.model.ts` | Rimuovere gli alias duplicati (`baseData`/`external` → solo `external`; `memories`/`personal` → solo `personal`); rimuovere i campi calcolati che non appartengono più a `TravelPlace` (nessuno oggi presente, verificare in implementazione) | Basso — tipo isolato, nessun consumer attivo lo usa oggi | Nessuna |
| 2 | `src/domain/trip/engine/PlaceMergeEngine.ts` | Standardizzare `mergePlace()` (già esistente, righe 134-193) su `mergeFromProvider(existing, incoming: PlaceMetadata): TravelPlace` — rimuovere il ramo union `ExternalPlace`, i nove cast `as PlaceMetadata` associati, e il type-guard runtime oggi necessario per disambiguarli; `isSamePlace`/`calculateDistanceMeters`/`calculateNameSimilarity` restano invariate e riusabili | Medio — è il cuore della nuova pipeline, va coperto da test prima di collegare i chiamanti (oggi `PlaceMergeEngine.test.ts` copre solo i metodi statici di dedup, non `mergePlace`) | #1 |
...
| 5 | `app/trip/[id]/places/[placeId].tsx` | Rimuovere interamente il fallback `placeRepository.getPlaceDetails()`; il luogo si legge solo tramite `usePlaces(tripId)` | Basso — elimina 24/26 errori tsc odierni, comportamento di fallback va sostituito con una UI di stato vuoto se il luogo non è tra i salvati | #3 |
...
| 11 | `src/features/places/catalog/editorial-places.catalog.ts` | Ogni `EditorialPlaceItem` diventa input di `PlaceMergeEngine.mergeFromProvider` con livello Editorial pre-popolato, invece di essere smontato a mano in `PlaceRef` in `places/index.tsx` | Basso — dati statici, nessuna scrittura utente coinvolta | #2, #6 |
