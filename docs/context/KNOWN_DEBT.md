...
## Frammentazione del modello Place — ora verificata a livello di compilatore, priorità alta

`DOMAIN_MODEL.md` documenta quattro rappresentazioni parallele di "un luogo" (`Place` del backend/`core/domain`, `TravelPlace` Zod, `PlaceRef`, `JourneyPlace`), presentandole come un trade-off deliberato. Il 2026-07-07 questo trade-off ha mostrato il suo costo reale: `npx tsc --noEmit` sulla working tree riporta **26 errori**, 24 dei quali in un solo file — `app/trip/[id]/places/[placeId].tsx` — dove il fallback a `placeRepository.getPlaceDetails()` (bypass noto della regola "solo hook", vedi [ARCHITECTURE.md](../architecture/ARCHITECTURE.md)) restituisce un `Place` che viene poi trattato come se fosse un `PlaceRef` (proprietà mancanti: `category`, `isVisited`, `notes`, `coverImageUrl`, `durationMinutes`, `address`...).

**Non è un errore da correggere riga per riga.** È la stessa causa radice della duplicazione `TravelPlace`/`PlaceRef` già nota — qui si manifesta al confine backend↔UI invece che al confine dei due percorsi di pianificazione. La correzione corretta è definire esplicitamente la pipeline di trasformazione tra le rappresentazioni (proposta in discussione: `Provider → Canonical Place → JourneyPlace → PlaceRef`, con un mapper esplicito ad ogni freccia) e farla seguire da un'ADR, coerente con lo stile con cui ADR-014 ha trattato una scelta di modello altrettanto strutturale. Vedi [CURRENT_ROADMAP.md](CURRENT_ROADMAP.md) per la sequenza concordata.
...
## Anchor bagagli di partenza non riconciliati con transfer/aeroporto — TODO Sprint 18 Fase 6

Emerso durante Sprint 18 Fase 5 (merge degli anchor bagagli in `JourneyAnchorEngine.buildTripAnchors()`). `LuggagePlanningService` è — e deve restare — **transport-agnostic**: posiziona il `luggage_pickup` di partenza a `partenza − 15min` e l'`accommodation_return` subito prima, **ignorando** i buffer che `JourneyAnchorEngine` applica alla partenza (`departure_transfer` = 45', `departure_airport` = 120'). Di conseguenza, in un round-trip la timeline unita può mostrare `accommodation_return`/`luggage_pickup` **cronologicamente dopo** `departure_airport` — il viaggiatore risulta "già in aeroporto" prima di tornare in hotel a recuperare i bagagli. Verificato in [JourneyAnchorEngine.ts](../../src/domain/services/JourneyAnchorEngine.ts) (`mergeChronologically`, ordina soltanto per `startISO`, non riconcilia le finestre) e coperto dai test di merge in [JourneyAnchorEngine.test.ts](../../src/domain/services/JourneyAnchorEngine.test.ts) (che asseriscono solo la monotonicità, non la correttezza semantica di questa transizione).

**Non è un bug di Fase 5**: la fase era esplicitamente "merge & sort", non riconciliazione (vedi [SPRINT_18_HUMAN_TRAVEL_LOGIC.md §7](../architecture/SPRINT_18_HUMAN_TRAVEL_LOGIC.md) e ADR-023). Debito noto, bloccato dal comportamento atteso della fase.

**TODO Fase 6 — riconciliazione degli anchor di partenza**:
- `LuggagePlanningService` **resta transport-agnostic** (nessuna conoscenza di transfer/aeroporto: invariante di dominio da preservare).
- La riconciliazione avviene in `JourneyAnchorEngine` (o nel futuro `EndOfDayClosureService`, ADR-024 §2.3), spostando il `luggage_pickup` (e il relativo `accommodation_return`) **prima dell'inizio della finestra di `departure_transfer`**, non prima dell'istante di volo.
- Attenzione alla collisione di significato su `accommodation_return`: qui è il rientro *per recuperare i bagagli*; ADR-024 lo riserva anche alla chiusura di fine giornata (Regola C) calcolata da `EndOfDayClosureService`. Gli id oggi differiscono, ma la riconciliazione deve evitare doppioni.
