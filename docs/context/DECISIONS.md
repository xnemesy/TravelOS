...
[docs/adr/017-unificazione-modello-place.md](../adr/017-unificazione-modello-place.md)

**Dichiara**: `Proposta — decisione architetturale, nessuna implementazione`. **Verificato**: coerente, nessun codice ancora modificato. Analizza a fondo (inclusa una run diretta di `npx tsc --noEmit`, 26 errori confermati) le sei rappresentazioni di "luogo" oggi coesistenti — non quattro come nell'ipotesi di partenza in [CURRENT_ROADMAP.md](CURRENT_ROADMAP.md) — e **corregge** quella pipeline: elimina `JourneyPlace` (confermato codice morto, zero consumer), unifica i due percorsi Provider indipendenti e mai riconciliati (`PlaceMetadata` via SIP vs `Place`/`placeRepository`, quest'ultimo causa diretta di 24 dei 26 errori `tsc`), ed elegge `TravelPlace` (rivisto, senza gli alias duplicati `baseData`/`external` e `memories`/`personal`) a Canonical Place. Pipeline decisa: `PlaceMetadata → TravelPlace → PlaceRef`, due mapper dichiarati (`PlaceMergeEngine.mergeFromProvider`, standardizzazione di `mergePlace()` già esistente; `canonicalPlaceToPlaceRef`, nuovo). Include piano di migrazione file-per-file e la decisione esplicita, richiesta da [KNOWN_DEBT.md](KNOWN_DEBT.md), di rimuovere il percorso di pianificazione legacy (`PlannerEngine`/`usePlannerStore`/`TravelPlace` pre-revisione).
